...
def create_picture(data_result):
    try:
        ...
        query = f"SELECT picture_id FROM pictures WHERE project_id = '{project_id}' and data_id = '{data_id}'"
...
def build_metadata(data_result, picture_id_array):
    # Exclusion fields
    exclusions = [
        '_id',
        'formhub/uuid',
    ...
    try:
        ...
        if not transaction_row and project_result is not None:
            ...
            data = data['results']
            data_metadata_array = []
            for value in picture_id_array:
                tableName = 'pictures'
                query = f"SELECT picture_id, instance, name, url, ipfshash FROM {tableName} WHERE index = '{value}';"
                picture_row = read_query(query)
                picture_id = picture_row[0]
                instance = picture_row[1]
                name = picture_row[2]
                url = picture_row[3]
                ipfshash = picture_row[4]
                tableName = 'data'
                query = f"SELECT measurement, value FROM {tableName} WHERE id = '{data_id}' and project_id='{project_id}' and _id='{_id}' and file_name='{name}';"
                measurement_row = read_query(query)
                measurement = measurement_row[0]
                value = measurement_row[1]
                # Build the metadata
                data_metadata = {}
                data_metadata[measurement] = value
                data_metadata[str(measurement) + '_support_file'] = ipfshash
    ...
    try:
        ...
        if data_results != [] or data_results is not None:
            for data_result in data_results:
                ...
                # Build metadata 
                index = build_metadata(data_result, picture_id_array)
